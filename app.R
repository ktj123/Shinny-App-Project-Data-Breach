    ...
    sidebarLayout(
      ...
      sidebarPanel(
        selectInput(inputId = "Sta", 
                    label = "State:",
                    choices = c("NJ","CA","TX","NY","IL","MI","CT",
        ...
        sliderInput(inputId = "size", 
                    label = "Choose Line Size:", 
                    min = 0, max = 2.5, 
                    value = 1),
        # Set line type
        selectInput(inputId = "type", 
                    label = "Choose Line type:",
                    choices = c("solid", "twodash", "longdash", "dotdash", "dashed"), 
                    selected = "solid"),
        # Set font size
        ...
        mainPanel(
            tabsetPanel(
            tabPanel("Data Breach Cases by States", plotOutput(outputId = "Bars1")),
            tabPanel("Data Breach Cases in US", plotOutput(outputId = "Bars2")),
            tabPanel("Data Breach Trend by States", 
                    plotOutput(outputId ="Lines1.1"),
                    plotOutput(outputId ="Lines1.2")),
            tabPanel("Data Breach Trend in US", 
                    plotOutput(outputId ="Lines2.1"),
                    plotOutput(outputId ="Lines2.2")),                    
...
server<-function(input, output) {
    ...
    output$Bars1 <- renderPlot({
        d1<-subset(dat1())
        barplot(table(d1[,input$x]),
                main=paste("Data Breach Cases in", input$Sta),
    ...
    output$Bars2 <- renderPlot({
        barplot(table(PRC_Data_Breach_Chronology[,input$x]),
                main="Data Breach Cases in United States",
                ylab="Number of Cases",
                xlab=input$x,
                col=input$color_b)    
    })
    
    # Scatter Plots by states
    output$Lines1.1 <- renderPlot({
      d1<-subset(dat1())
      ggplot(d1,aes(Year,color=Type_of_breach))+geom_freqpoly(linetype =input$type, size=input$size,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = paste("Data Breach Frenquency in", input$Sta), y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })    
    output$Lines1.2 <- renderPlot({
      d1<-subset(dat1())    
      ggplot(d1,aes(Year,color=Type_of_organization))+geom_freqpoly(linetype =input$type,size=input$size,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = paste("Data Breach Frenquency in", input$Sta), y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })
    ...
    # Scatter Plots in US
    output$Lines2.1 <- renderPlot({
      ggplot(PRC_Data_Breach_Chronology,aes(Year,color=Type_of_breach))+geom_freqpoly(linetype =input$type,size=input$size,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = "Data Breach Frenquency in United States", y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })    
    output$Lines2.2 <- renderPlot({
      ggplot(PRC_Data_Breach_Chronology,aes(Year,color=Type_of_organization))+geom_freqpoly(linetype =input$type,size=input$size,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = "Data Breach Frenquency in United States", y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })    
