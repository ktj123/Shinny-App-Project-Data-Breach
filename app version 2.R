    ...
    sidebarLayout(
      ...
      sidebarPanel( width = 3,
        ...
        selectInput(inputId = "color_l", 
                    label = "Choose line Color (not for Frequency Chart)",
                    choices = c("red", "green", "grey", "yellow", "darkblue","pink","purple"), 
                    selected = "darkblue"),
        
        # Set line size
        sliderInput(inputId = "size_l", 
                    label = "Choose line Size: (not for Frequency Chart)", 
                    min = 0, max = 4, 
                    value = 1),
        
        # Set alpha level
        sliderInput(inputId = "alpha", 
                    label = "line Transparency: (not for Frequency Chart)", 
                    min = 0, max = 1, 
                    value = 1),
        
        # Set line type
        selectInput(inputId = "type_l", 
...
server<-function(input, output) {
    ...
    output$Lines1 <- renderPlot({
      ...
      ggplot(d3,aes(x=Year,y=count1))+
        stat_summary(fun= sum, geom="line",colour=input$color_l, size=input$size_l, alpha=input$alpha)+
        geom_text(stat = "count", aes(label = ..count.., y = ..count..), vjust =-0.5, size = 5)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = paste("Data Breach Trend in", input$Sta), y="Number of Cases")+
    ...
    output$Lines3.1 <- renderPlot({
      ...
      ggplot(d1,aes(Year,color=Type_of_breach))+
        geom_freqpoly(linetype =input$type_l, size=input$size_l,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = paste("Data Breach Cases in", input$Sta,"by Type"), y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })    
    output$Lines3.2 <- renderPlot({
      ...
      ggplot(d1,aes(Year,color=Type_of_organization))+
        geom_freqpoly(linetype =input$type_l,size=input$size_l,binwidth = 1)+
        xlim(as.numeric(input$min), as.numeric(input$max))+
        labs(title = paste("Data Breach Cases in", input$Sta,"by Organization"), y="Number of Cases")+
        theme_bw()+theme(text = element_text(size = input$font))
    })
